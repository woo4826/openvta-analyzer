...
- [x] **Step 1: Extend the MapLibre test double and write a failing handler test**
...
on(event: string, layerOrHandler: string | (() => void), handler?: (event: unknown) => void) {
  ...
  if (event !== "load" && typeof layerOrHandler === "function") this.globalHandlers.set(event, layerOrHandler);
  if (typeof layerOrHandler === "string" && handler) this.layerHandlers.set(`${event}:${layerOrHandler}`, handler);
  return this;
}
```

Add a test that invokes the generic MapLibre click handler with `features[0].properties.id = "wrong"` but a click coordinate on the second section. Assert `onSectionSelect` receives the coordinate-resolved ID and a finite distance:

```ts
map.globalHandlers.get("click")?.({
...
- [x] **Step 2: Run the RouteMap tests and confirm failure**

Run: `corepack pnpm test -- src/components/__tests__/RouteMapSources.test.tsx`

Expected: FAIL because the handler still relies on delegated feature hits/`features[0]` and fallback emits only an ID.

- [x] **Step 3: Update RouteMap callback and handler refs**
...
Do not read `event.features` for the selected ID. Listen to the generic map click so transparent-layer hit testing cannot drop taps, emit projection offset, and ignore clicks more than 100 m from the analysis line. For fallback polylines, find the matching `TrackSection`, call `sectionMidpointSelection`, and emit both arguments for click and keyboard activation.

- [x] **Step 4: Run RouteMap tests, typecheck, and lint the file**
...
- [x] **Step 1: Expose the mocked callback and write failing cursor tests**
...
- [x] **Step 2: Run the SegmentTrajectoryMap tests and confirm failure**
...
- [x] **Step 3: Implement one section-selection wrapper**
...
- [x] **Step 1: Write failing Workbench coordinate-frame and state tests**
...
- [x] **Step 2: Extend the LapAnalysis round-trip test**
...
- [x] **Step 3: Run both test files and confirm failure**
...
- [x] **Step 4: Add controlled selection synchronization**
...
- [x] **Step 5: Run component tests, typecheck, and lint**
...
- [x] **Step 1: Add a browser state round-trip regression**
...
- [x] **Step 3: Verify the supplied VTA with Aside**
...
- [x] **Step 4: Review the complete diff and commit verification changes**
...
- Supplied file: `VTA24082025_101142_CC00.Vta`; parsed 1,589 GPS fixes and 158,289 sensor rows; matched built-in `Inje Speedium`.
- Actual preset-to-driven-path projection offsets sampled at seven separated map locations: 4.27–75.39 m. The final generic-click threshold is 100 m; a clear off-track click remained on Whole Lap.
- Actual map clicks selected and synchronized `Straight 2 · 243–273 m` at cursor 10 m, `Corner 1 · 0–93 m` at 40 m, `Corner 13 · 3393–3518 m` at 55 m, and `Corner 11 · 2998–3118 m` at 0 m.
- Actual state round trip: map `inje-c13` → Setup editor `inje-c13` → Setup selection `inje-c7` → Workbench `Corner 7 · 2143–2283 m`.
- Aside page errors: 0. Aside console errors during the representative round trip: 0.
- Fresh automated gate after the final 100 m tolerance: typecheck PASS, lint PASS, Vitest 59 files / 334 tests PASS, production build PASS, Playwright desktop/mobile 16 tests PASS, `git diff --check` PASS.
