...
- Current lap-analysis scope CSV and versioned JSON.
...
The deploy includes a small versioned track index under `public/tracks/`. The app filters its bounds and length metadata before fetching candidate profiles, validates every downloaded TrackProfile, and safely falls through when an asset is unavailable. Hosted files provide defaults only; edits are persisted separately as local overrides.

Track profiles are cached in the browser's IndexedDB database `openvta-analyzer`, in the `track-profiles` object store, with origin metadata in `track-profile-origins`. Track Library catalog import uses one transaction. If IndexedDB is unavailable or blocked, an in-memory fallback keeps the complete validated batch usable for the current page session, and JSON import/export remains available. Cached profiles are scored against each new recording. A fresh OSM profile (less than 30 days old) avoids a network lookup; a stale match can remain visible while the app refreshes it.
